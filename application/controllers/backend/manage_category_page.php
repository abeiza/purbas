<?php 
	...
	class Manage_Category_Page extends CI_Controller{
		...
		function search(){
			...
			$search = $this->db->query("select * from purb_page_category where page_category_name like '%".$this->session->userdata('search_page_category')."%' or page_category_description like '%".$this->session->userdata('search_page_category')."%'");
			...
			$data['category_grid'] = $this->db->query("select * from purb_page_category where page_category_name like '%".$this->session->userdata('search_page_category')."%' or page_category_description like '%".$this->session->userdata('search_page_category')."%' LIMIT ".$limit." OFFSET ".$offset."");
			$data['limit'] = $limit;
